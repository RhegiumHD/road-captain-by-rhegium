...
async function publicAutocomplete(input) {
  ...
  // Photon/KOMOOT fallback: fast and useful for typed Italian addresses.
  try {
    const url = "https://photon.komoot.io/api/?limit=7&lang=it&q=" + encodeURIComponent(q);
    const data = await fetchJson(url);
    const items = (data.features || []).map(f => {
      const p = f.properties || {};
      const parts = [p.name, p.street, p.housenumber, p.city, p.county, p.state, p.country]
        .filter(Boolean);
      return {
