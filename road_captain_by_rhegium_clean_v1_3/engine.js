...
export function addStopEtas(stops, routeDurationMinutes, routeKm, baseDate, mode = "depart", requestedTime = "09:00") {
  ...
  const standardStopMinutes = valid.reduce((sum, s) => sum + Number(s.durationMinutes || 20), 0);
  const totalMinutes = routeDurationMinutes + standardStopMinutes;
  ...
  const withEta = stops.map(s => {
    ...
    const eta = new Date(departure.getTime() + (routeDurationMinutes * driveShare + extraBefore) * 60000);
    extraBefore += Number(s.durationMinutes || 20);
    const etaMinuteOfDay = eta.getHours() * 60 + eta.getMinutes();
    return { ...s, etaIso: eta.toISOString(), etaMinuteOfDay };
  });
