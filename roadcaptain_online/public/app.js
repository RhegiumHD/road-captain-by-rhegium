...
const ICONS = {
  ...
  rest: _svg('<rect x="6.5" y="4" width="3.6" height="16" rx="1.4"/><rect x="13.9" y="4" width="3.6" height="16" rx="1.4"/>', {fill:"currentColor", stroke:"none"})
};
...
function renderMap(data){
  ...
  (data.stops || []).forEach((s, i) => {
    ...
    const isFuel = s.kind === "fuel";
    const cls = critical ? "rc-critical-marker" : (isFuel ? "rc-stop-marker" : "rc-rest-marker");
    const glyph = critical ? ICONS.alert : (isFuel ? ICONS.fuel : ICONS.rest);
    const kindTxt = critical ? "" : `<br><small>${isFuel ? "Rifornimento" : "Sosta tecnica · senza rifornimento"}</small>`;
    const m = L.marker([s.lat,s.lng], {icon:baseIcon(glyph, cls, 36)}).bindPopup(`<strong>${label}</strong><br>${s.displayName || s.name || s.message || ""}<br><small>km ${Number(s.routeKm||0).toFixed(1)}</small>${kindTxt}`).addTo(map);
...
function addPoiToList(p){
  const k = poiKey(p);
  if (addedPois.some(x => x.key === k)) return false;
  if (!Number.isFinite(Number(p.lat)) || !Number.isFinite(Number(p.lng))) return false;
  addedPois.push({key:k, lat:Number(p.lat), lng:Number(p.lng), routeKm:Number(p.routeKm)||0, label:p.name||"POI"});
  return true;
}
// Aggiunge in UN colpo solo tutti i POI selezionati (un solo ricalcolo, non uno per POI).
function addSelectedPois(){
  const checks = Array.from(document.querySelectorAll("#routePois .poiSel:checked"));
  let added = 0;
  checks.forEach(c => { const p = lastRoutePois[Number(c.dataset.i)]; if (p && addPoiToList(p)) added++; });
  if (!added) return;
  renderAddedPoiBar();
  runCalc();
}
...
const _selPoiBtn = $("addSelectedPois"); if (_selPoiBtn) _selPoiBtn.onclick = addSelectedPois;
...
(function(){
  ...
  // Web3Forms va chiamato DAL BROWSER (com'e' progettato): dal server Render Cloudflare
  // risponde 403 "Just a moment". La chiave e' pubblica per design; il destinatario resta
  // legato alla chiave e non viene mai esposto qui.
  const FEEDBACK_KEY = "7f7450b4-6cc9-4093-ade6-4336d47419e3";
    ...
    try {
      const r = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          ...
          subject: "Road Captain - nuovo feedback",
          from_name: "Road Captain by Rhegium",
          message: message + "\n\n— Contatto lasciato: " + (contact || "(nessuno)")
        })
      });
      const d = await r.json().catch(()=>({}));
      delivered = !!d.success;
      info = d.message || ("HTTP " + r.status);
    } catch(e){ info = "rete: " + (e && e.message || e); }
    // Backup lato server (solo memoria, leggibile da /api/admin/feedback). Best-effort.
    try { fetch("/api/feedback", {method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({message, contact})}); } catch(e){}
    if (delivered){ $("fbStatus").textContent = "Grazie! Feedback inviato."; $("fbMessage").value=""; $("fbContact").value=""; setTimeout(close, 1500); }
    else { $("fbStatus").textContent = "Non inviato (" + info + "). Riprova."; }
...
function googleMapsUrl(d){
  ...
  if (!o || !de) return null;
  // Google Maps accetta poche tappe nell'URL (max 9): includo SIA le soste pianificate
  // SIA i POI inseriti, ordinati per posizione sulla rotta. Se sono piu' di 9 si campiona
  // uniformemente per non perdere la distribuzione lungo il percorso.
  const pts = [];
  (d.stops||[]).forEach(s=>{ if (s.status!=="CRITICA" && Number.isFinite(s.lat) && Number.isFinite(s.lng)) pts.push({lat:s.lat, lng:s.lng, km:Number(s.routeKm||0)}); });
  addedPois.forEach(x=>{ if (Number.isFinite(x.lat) && Number.isFinite(x.lng)) pts.push({lat:x.lat, lng:x.lng, km:Number(x.routeKm||0)}); });
  pts.sort((a,b)=>a.km-b.km);
  let picked = pts;
  if (pts.length > 9) {
    picked = [];
    for (let i=0;i<9;i++) picked.push(pts[Math.round(i*(pts.length-1)/8)]);
  }
  const wp = picked.map(p=>`${p.lat},${p.lng}`).join("|");
...
async function runCalc(){
  ...
  try{
    ...
    $("stops").innerHTML=d.stops.map(s=>{
      ...
      const isFuel = s.kind === "fuel";
      const kindTag = `<span class="kindTag ${isFuel ? "kindFuel" : "kindRest"}">${isFuel ? "Rifornimento" : "Sosta tecnica · senza rifornimento"}</span>`;
      ...
      `<div class="stop"><span class="badge ${s.status}">${s.status}</span><strong>km ${s.routeKm.toFixed(1)} totali</strong>${prev}<h3>${s.displayName || s.name}</h3>${kindTag}<p>${s.address}</p>${leg}${eta}${stopDuration}${long}${departureEta}<small>${s.brand && s.brand !== (s.displayName || s.name) ? s.brand + " · " : ""}${s.source} | scostamento rotta ${s.offsetMeters} m</small></div>`
    }).join("") || "Nessuna sosta necessaria.";
    const routePois = d.routePois || [];
    lastRoutePois = routePois;
    const routeKeys = new Set(routePois.map(poiKey));
    // POI aggiunti come tappa ma non piu' nella lista ricalcolata: restano "fissati" in cima.
    const pinnedHtml = addedPois.filter(x => !routeKeys.has(x.key)).map(x =>
      `<div class="poiItem isAdded pinnedPoi"><span class="poiIco">${poiIcon("historic")}</span><div class="poiBody"><strong>${escHtml(x.label)}</strong><p><small>Tappa aggiunta all'itinerario · km ${Number(x.routeKm||0).toFixed(1)}</small></p><a class="poiLink" href="https://www.google.com/maps/search/?api=1&query=${x.lat},${x.lng}" target="_blank" rel="noopener">Apri in Maps</a><button type="button" class="poiAdd added" data-key="${escHtml(x.key)}">✓ Rimuovi dall'itinerario</button></div></div>`
    ).join("");
    let selectableCount = 0;
    const routeHtml = routePois.map((p,i)=>{
      const wiki = p.wikipediaUrl || `https://it.wikipedia.org/w/index.php?search=${encodeURIComponent(p.name||"")}`;
      ...
      if (canAdd && !added) selectableCount++;
      // Gia' aggiunto -> tasto Rimuovi; altrimenti checkbox di selezione (aggiunta in blocco).
      const control = !canAdd ? "" : (added
        ? `<button type="button" class="poiAdd added" data-key="${escHtml(poiKey(p))}">✓ Rimuovi dall'itinerario</button>`
        : `<label class="poiSelLbl"><input type="checkbox" class="poiSel" data-i="${i}"> Seleziona</label>`);
      const desc = (p.description && p.description.length > 5) ? p.description : poiDescription(p);
      return `<div class="poiItem${added?" isAdded":""}"><span class="poiIco">${poiIcon(p.icon)}</span><div class="poiBody"><strong>${i+1}. ${escHtml(p.name || "Luogo di interesse")}</strong><p>${escHtml(p.address || "")}</p><small>km ${Number(p.routeKm||0).toFixed(1)} &middot; fuori rotta ${(Number(p.offsetMeters||0)/1000).toFixed(1)} km &middot; ${escHtml(p.category || "POI")} &middot; <span class="poiToggle">cos'è?</span></small><div class="poiDesc">${escHtml(desc)}<div class="poiLinks"><a href="${wiki}" target="_blank" rel="noopener">Wikipedia</a><a href="${maps}" target="_blank" rel="noopener">Apri in Maps</a></div></div>${control}</div></div>`;
    }).join("");
    $("routePois").innerHTML = (pinnedHtml + routeHtml) || "Nessun luogo notabile trovato lungo la rotta.";
    const selBtn = $("addSelectedPois");
    if (selBtn) selBtn.classList.toggle("hidden", selectableCount === 0);
    $("routePois").querySelectorAll(".poiItem").forEach(el => el.addEventListener("click", (e) => { if (e.target.closest("a") || e.target.closest(".poiAdd") || e.target.closest(".poiSelLbl")) return; el.classList.toggle("open"); }));
