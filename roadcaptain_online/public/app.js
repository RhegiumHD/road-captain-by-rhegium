...
let rcMap = null;
let rcMapLayers = [];
let rcMapResizeObserver = null;
function forceMapResize(delay=0){
  if (!rcMap) return;
  setTimeout(() => {
    try { rcMap.invalidateSize({pan:false}); } catch(e) {}
  }, delay);
}
...
function initMap(){
  const mapEl = $("map");
  if (rcMap || typeof L === "undefined" || !mapEl) return;
  rcMap = L.map(mapEl, {scrollWheelZoom:false}).setView([41.9, 12.5], 6);
  L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
    maxZoom: 19,
    attribution: "&copy; OpenStreetMap"
  }).addTo(rcMap);

  // Leaflet calcola la griglia delle tiles sulla dimensione del contenitore al momento
  // dell'inizializzazione. Con il layout a 3 colonne/sticky il pannello può assestarsi
  // dopo qualche frame: forziamo quindi il ricalcolo e lo ripetiamo ai resize reali.
  forceMapResize(0);
  forceMapResize(250);
  forceMapResize(800);
  window.addEventListener("resize", () => forceMapResize(100));
  if (typeof ResizeObserver !== "undefined") {
    rcMapResizeObserver = new ResizeObserver(() => forceMapResize(60));
    rcMapResizeObserver.observe(mapEl);
  }
}
function renderMap(data){
  ...
  (data.stops || []).forEach((s, i) => {
    ...
    const icon = L.divIcon({className: s.status === "CRITICA" ? "rc-critical-marker" : "rc-stop-marker", html:"⛽", iconSize:[34,34], iconAnchor:[17,17]});
    const m = L.marker([s.lat,s.lng], {icon}).bindPopup(`<strong>${label}</strong><br>${s.displayName || s.name || s.message || ""}<br><small>km ${Number(s.routeKm||0).toFixed(1)}</small>`).addTo(rcMap);
    rcMapLayers.push(m); bounds.push([s.lat,s.lng]);
  });
  (data.routePois || []).forEach((p, i) => {
    if (!Number.isFinite(p.lat) || !Number.isFinite(p.lng)) return;
    const icon = L.divIcon({className:"rc-poi-marker", html:"★", iconSize:[28,28], iconAnchor:[14,14]});
    const m = L.marker([p.lat,p.lng], {icon}).bindPopup(`<strong>POI ${i+1}: ${p.name || "Luogo di interesse"}</strong><br>${p.address || ""}<br><small>km ${Number(p.routeKm||0).toFixed(1)} · fuori rotta ${Math.round((p.offsetMeters||0)/100)/10} km</small>`).addTo(rcMap);
    rcMapLayers.push(m); bounds.push([p.lat,p.lng]);
  });
  forceMapResize(0);
  forceMapResize(150);
  if (bounds.length) {
    setTimeout(() => rcMap.fitBounds(bounds, {padding:[24,24]}), 180);
  }
}
initMap();
