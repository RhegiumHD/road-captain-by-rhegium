...
function markerIcon(kind){
  if (kind === 'stop') return L.divIcon({className:'rc-stop-marker', html:'⛽', iconSize:[34,34], iconAnchor:[17,17], popupAnchor:[0,-17]});
  if (kind === 'critical') return L.divIcon({className:'rc-critical-marker', html:'!', iconSize:[34,34], iconAnchor:[17,17], popupAnchor:[0,-17]});
  return L.divIcon({className:'rc-poi-marker', html:'•', iconSize:[18,18], iconAnchor:[9,9], popupAnchor:[0,-9]});
}
function renderMap(data){
  ...
  (data.pois || []).forEach(p => {
    ...
    const m = L.marker([p.lat,p.lng], {icon:markerIcon('poi'), zIndexOffset:100})
      .bindPopup(`<strong>POI carburante</strong><br>${p.name || 'Distributore'}<br><small>km ${Number(p.routeKm||0).toFixed(1)} | ${p.source || ''}</small>`).addTo(rcMap);
    rcMapLayers.push(m); bounds.push([p.lat,p.lng]);
  });
  (data.stops || []).forEach((s, i) => {
    if (!Number.isFinite(s.lat) || !Number.isFinite(s.lng)) return;
    const kind = s.status === 'CRITICA' ? 'critical' : 'stop';
    const m = L.marker([s.lat,s.lng], {icon:markerIcon(kind), zIndexOffset:500})
      .bindPopup(`<strong>Sosta ${i+1} - ${s.status}</strong><br>${s.name || s.message || ''}<br><small>km ${Number(s.routeKm||0).toFixed(1)}${Number.isFinite(s.fromPreviousKm) ? ' | +' + s.fromPreviousKm.toFixed(1) + ' km dalla precedente' : ''}</small>`).addTo(rcMap);
...
$("calcBtn").onclick=async()=>{
  ...
  try{
    ...
    $("stops").innerHTML=d.stops.map(s=>{
      ...
      `<div class="stop"><span class="badge ${s.status}">${s.status}</span><strong>km ${s.routeKm.toFixed(1)} totali</strong>${prev}<h3>${s.name}</h3><p>${s.address}</p><small>${s.source} | scostamento rotta ${s.offsetMeters} m</small></div>`
