...
// Sceglie l'area reale piu' adatta vicino a un km obiettivo. REGOLA DURA: mai fuori
// autostrada se esiste un'area autostradale. Il lato destro (carreggiata corretta) e' una
// PREFERENZA d'ordinamento, non un filtro che esclude: meglio l'area "gemella" a sinistra
// che un buco di centinaia di km. Finestra [target, target+margine] -> a ritroso -> avanti.
function sideRank(p){ return p.side === "right" ? 0 : (p.side === "unknown" ? 1 : 2); }
function pickAreaNear(sorted, {fromKm, target, forwardWindowKm, minGapKm, capKm=Infinity}) {
  ...
  if (onMw.length) pool = onMw;                 // REGOLA DURA: solo aree autostradali se ce ne sono
  if (!pool.length) return null;
  const within = pool.filter(p => p.routeKm >= target && p.routeKm <= target + Number(forwardWindowKm))
                     .sort((a,b)=> (sideRank(a)-sideRank(b)) || (Number(a.offsetMeters||0)-Number(b.offsetMeters||0)) || (a.routeKm-b.routeKm));
  const before = pool.filter(p => p.routeKm < target)
                     .sort((a,b)=> (b.routeKm - a.routeKm) || (sideRank(a)-sideRank(b)));   // piu' vicino sotto il target
  const after  = pool.filter(p => p.routeKm > target + Number(forwardWindowKm))
                     .sort((a,b)=> (a.routeKm - b.routeKm) || (sideRank(a)-sideRank(b)));   // piu' vicino sopra
  ...
  if (after.length)  return {area:after[0],  status:"VALIDA"};        // ultima risorsa: estende avanti
  return null;
}
