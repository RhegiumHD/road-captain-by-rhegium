...
export function planStops({routeKm, pois, stopEveryKm=150, forwardWindowKm=25, autonomyKm=null}) {
  ...
  // Rifinitura: niente due soste troppo vicine. Una sosta TECNICA (riposo) entro `mergeKm`
  // da un rifornimento e' inutile (ci si riposa al rifornimento stesso): si elimina. I
  // rifornimenti non si toccano mai (servono per l'autonomia). Cosi' si evitano i casi
  // assurdi tipo "sosta tecnica a 39 km dal rifornimento".
  const mergeKm = Math.max(70, every * 0.45);
  for (let idx = stops.length - 1; idx >= 0; idx--) {
    if (stops[idx].kind !== "normal") continue;
    const prevFuelGap = idx > 0 ? stops[idx].routeKm - stops[idx-1].routeKm : Infinity;
    const nextFuelGap = idx < stops.length-1 ? stops[idx+1].routeKm - stops[idx].routeKm : Infinity;
    const prevIsFuel = idx > 0 && stops[idx-1].kind === "fuel";
    const nextIsFuel = idx < stops.length-1 && stops[idx+1].kind === "fuel";
    if ((prevIsFuel && prevFuelGap < mergeKm) || (nextIsFuel && nextFuelGap < mergeKm)) stops.splice(idx, 1);
  }
  return stops;
}
