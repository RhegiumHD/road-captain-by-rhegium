...
function distPointSegmentKm(p, a, b) {
...
export function locateOnRouteKm(point, polyline, cum) {
  ...
  for (let i=1; i<polyline.length; i++) {
    const r = distPointSegmentKm(point, polyline[i-1], polyline[i]);
...
export function planFuelStops({routeKm, pois, stopEveryKm=150, forwardWindowKm=25, maxAutonomyKm=200}) {
  ...
  while (routeKm - lastKm > maxAutonomyKm) {
    ...
    if (!candidates.length) {
      stops.push({status:"CRITICA", routeKm: Math.round(target), message:`Nessuna stazione carburante valida sulla rotta tra km ${Math.round(lastKm)} e km ${Math.round(Math.min(lastKm+maxAutonomyKm, routeKm))}. Serve verifica manuale.`});
      break;
    }
