...
export function planFuelStops({routeKm, pois, stopEveryKm=150, forwardWindowKm=25, maxAutonomyKm=200}) {
  ...
  while (routeKm - lastKm > maxAutonomyKm) {
    ...
    if (candidates.length) {
      // Preferisci la stazione piu' "sulla rotta" (scostamento minore), poi la piu' vicina al target.
      candidates.sort((a,b)=>(Number(a.offsetMeters||0)-Number(b.offsetMeters||0)) || (a.routeKm-b.routeKm));
    } else {
      candidates = sorted.filter(p => p.routeKm > lastKm + minGapKm && p.routeKm <= target)
        .sort((a,b)=>(Number(a.offsetMeters||0)-Number(b.offsetMeters||0)) || (b.routeKm-a.routeKm));
      status = "ANTICIPATA";
    }
    if (!candidates.length) {
