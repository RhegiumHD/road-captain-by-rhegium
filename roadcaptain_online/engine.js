...
export function planStops({routeKm, pois, stopEveryKm=150, forwardWindowKm=25, autonomyKm=null}) {
  ...
  // --- Fase 1: soste a cadenza (normali) ---
  const stops = [];
  let lastKm = 0;
  while (true) {
    const target = lastKm + every;
    if (target > routeKm - minGapKm) break;        // troppo vicino all'arrivo: niente sosta
    const pick = pickAreaNear(sorted, {fromKm:lastKm, target, forwardWindowKm:fwd, minGapKm});
    if (!pick) break;                              // nessuna area utile piu' avanti
    stops.push({...pick.area, status:pick.status, kind:"normal"});
    lastKm = pick.area.routeKm;
