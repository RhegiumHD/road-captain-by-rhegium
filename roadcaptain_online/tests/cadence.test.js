...
// 5) MERGE soste tecniche troppo vicine a un rifornimento (caso "TUGO a 39 km da ENILIVE").
// Aree fitte; cadenza piccola (genera soste tecniche) + autonomia che forza rifornimenti:
// una sosta tecnica a ridosso di un rifornimento deve sparire.
const densePois = [];
for (let km = 30; km < 1300; km += 20) densePois.push({name:"A"+km, routeKm:km, offsetMeters:40, onMotorway:true, side:"right"});
const merged = planStops({routeKm:1300, pois:densePois, stopEveryKm:150, forwardWindowKm:25, autonomyKm:270});
for (let i=1;i<merged.length;i++){
  const gap = merged[i].routeKm - merged[i-1].routeKm;
  const oneIsTech = merged[i].kind === "normal" || merged[i-1].kind === "normal";
  if (oneIsTech && gap < 60) throw new Error("sosta tecnica troppo vicina a un'altra: "+gap.toFixed(0)+" km");
}
console.log("cadence-merge ok");
