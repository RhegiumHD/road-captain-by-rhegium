...
async function nearbyGasGoogle(lat, lng, fuelType) {
  const body = {
    ...
    maxResultCount: 20,
    rankPreference: "DISTANCE",          // i piu' vicini al punto sul tracciato (= le aree autostradali) per primi
...
async function collectFuelPois(polyline, cum, routeKm, fuelType) {
  const seen = new Map();
  // Campionamento FITTO: il passo deve essere <= 2x il raggio di ricerca (8 km) o restano
  // tratti mai cercati. A 35 km con raggio 6 km si saltavano ~23 km per volta (meta' degli
  // autogrill non veniva nemmeno interrogata). 14 km -> copertura piena della rotta.
  const sampleStep = 14;
  const points = [];
