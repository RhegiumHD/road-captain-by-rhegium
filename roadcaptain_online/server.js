...
async function getJson(url, options={}) {
  const headers = {"User-Agent":"RoadCaptainByRhegium/3.1.2", ...(options.headers||{})};
...
async function api(req,res,url) {
  ...
  if (url.pathname === "/api/fuel") {
    ...
    return send(res,200,{ok:true,type,price:prices[type]||1.85,source:"Prezzo suggerito aggiornato. Puoi modificarlo manualmente."});
  }
  if (url.pathname === "/api/plan" && req.method === "POST") {
    ...
    req.on("end", async()=>{
      try {
        ...
        return send(res,200,{ok:true, mode:GOOGLE_KEY?"Google":"Fallback gratuito", geocoded:{origin,destination}, route:rt, poisFound:pois.length,
...
server.listen(PORT,()=>console.log(`Road Captain by Rhegium 3.1.2 on ${PORT}`));
