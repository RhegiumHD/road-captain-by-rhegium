...
async function routeFallback(origin, destination) {
...
async function route(origin, destination) {
  return GOOGLE_KEY ? routeGoogle(origin,destination) : routeFallback(origin,destination);
}
  ...
  if (url.pathname === "/api/plan" && req.method === "POST") {
    ...
    req.on("end", async()=>{
      try {
        ...
        return send(res,200,{ok:true, mode:GOOGLE_KEY?"Google":"Fallback gratuito", geocoded:{origin,destination}, route:rt, tolls, poisFound:pois.length, pois:poisForMap,
          times:{departure:fmtRome(departure),arrival:fmtRome(arrival),stopMinutes:validStops*Number(b.stopMinutes||15)},
...
if (import.meta.url === pathToFileURL(process.argv[1] || "").href) {
  server.listen(PORT,()=>console.log(`Road Captain by Rhegium 3.1.10 on ${PORT}`));
}
