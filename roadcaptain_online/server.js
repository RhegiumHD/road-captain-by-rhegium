...
async function collectRouteAttractions(polyline, cum, routeKm) {
  ...
  for (let km = 15; km < routeKm; km += sampleStep) { const p = pointAtKm(polyline, cum, km); if (p) points.push(p); }
  const batches = await mapLimit(points, 6, async (p) => {
    try { return GOOGLE_KEY ? await nearbyAttractionsGoogle(p.lat, p.lng) : await nearbyAttractionsFallback(p.lat,p.lng); } catch { return []; }
  });
  ...
  if (url.pathname === "/api/feedback" && req.method === "POST") {
    ...
    req.on("end", async()=>{
      try {
        ...
        let delivered = false, diag = "chiave non impostata";
        // Chiave Web3Forms: prima l'env (per poterla ruotare senza toccare il codice),
        // altrimenti la chiave del modulo (e' una chiave pubblica lato client, il destinatario
        // resta legato alla chiave e non e' esposto qui).
        ...
        if (key) {
          try {
            ...
            delivered = (w3 && w3.success === true);
            diag = (w3 && (w3.message || (delivered ? "ok" : "rifiutato"))) || "nessuna risposta";
          } catch (e) { diag = "errore rete/Web3Forms: " + (e && e.message || e); }
        }
        entry.delivery = {delivered, diag};   // visibile da /api/admin/feedback per diagnosi
        return send(res, 200, {ok:true, delivered, diag, note: delivered ? "Feedback inviato." : "Feedback registrato (email non recapitata)."});
  ...
  if (url.pathname === "/api/plan" && req.method === "POST") {
    ...
    req.on("end", async()=>{
      try {
        ...
        const [pois, routePois] = await Promise.all([
          collectFuelPois(rt.polyline,cum,rt.distanceKm,b.fuelType||"benzina"),
          collectRouteAttractions(rt.polyline,cum,rt.distanceKm)
        ]);
        ...
        return send(res,200,{ok:true, mode:GOOGLE_KEY?"Google":"Fallback gratuito", geocoded:{origin,destination,waypoints}, route:rt, tolls, tollNotice, poisFound:pois.length, pois:poisForMap, routePois,
          ...
          fuel:{type:b.fuelType||"benzina",price:fuelPrice,liters,cost:fuelCost,tankCapacityL,consumptionKmL:consumption,autonomyKm}, stops});
      } catch(e) { return fail(res,500,e.message); }
    });
    return;
  }
