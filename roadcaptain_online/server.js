...
async function collectFuelPois(polyline, cum, routeKm, fuelType) {
  ...
  for (const pois of batches) {
    for (const poi of pois) {
      ...
      // Soglia stretta (150 m): esclude i benzinai su strade parallele, che pur
      // essendo vicini in linea d'aria richiederebbero di uscire dal percorso.
      if (loc.distKm <= 0.15) {
        const key = poi.id || `${poi.name}-${poi.lat.toFixed(4)}-${poi.lng.toFixed(4)}`;
        // Segnale "sull'asse autostradale": dall'indirizzo/nome (Autostrada, A1, area di
        // servizio, km autostradale). Distingue una vera area dal benzinaio su strada
        // parallela vicino in linea d'aria ma raggiungibile solo uscendo.
        const onMotorway = /autostrada|\ba\d{1,2}\b|raccordo|area\s+(?:di\s+)?servizio/i.test(`${poi.name||""} ${poi.address||""}`);
        // Lato di marcia: in Italia si guida a destra, quindi l'area di servizio giusta
        // e' a DESTRA del senso di marcia. Quella della carreggiata opposta cade a sinistra.
        // Distingue la tua area da quella "gemella" sull'altra corsia (vicina ma inaccessibile).
        const a = pointAtKm(polyline, cum, Math.max(0, loc.km - 0.3));
        const c = pointAtKm(polyline, cum, loc.km + 0.3);
        let side = "unknown";
        if (a && c) {
...
// "Visibile di passaggio": castelli, torri, fari, punti panoramici, monumenti, chiese/abbazie,
// riserve/parchi naturali, attrazioni generiche. ESCLUDE ciò che richiede di fermarsi ed entrare
// (musei, cantine, terme/spa, acquari, zoo, parchi acquatici/divertimenti, hotel, ristoranti, negozi).
export function isLandmarkVisible(item) {
  ...
  if (/castell|castle|fort|torre|tower|faro|lighthouse|ponte|bridge|monument|memorial|riferimento|landmark|panoram|belvedere|viewpoint|scenic|chiesa|church|duomo|basilica|cattedral|abbazia|abbey|santuario|tempio|riserva|parco nazionale|national_park|cascat|gola|lago|montagn|attrazione turistica|tourist_attraction|punto di interesse|point_of_interest/.test(t)) return true;
  return false;
}
...
async function collectRouteAttractions(polyline, cum, routeKm) {
  ...
  for (let km = 15; km < routeKm; km += sampleStep) { const p = pointAtKm(polyline, cum, km); if (p) points.push(p); }
  const batches = await mapLimit(points, 6, async (p) => {
    // POI SEMPRE da Overpass/OSM (gratis): i POI sono una chicca, non valgono la SKU
    // Places Enterprise a pagamento. Il carburante resta su Google (tier Pro, gratuito) per
    // i nomi-area di qualita'. Cosi' la spesa Places Enterprise e' strutturalmente ZERO.
    try { return await nearbyAttractionsFallback(p.lat, p.lng); } catch { return []; }
  });
  for (const items of batches) {
    for (const item of items) {
      ...
      // allargando fino a 8 km solo dove un segmento non ha nulla di piu' vicino.
      if (loc.distKm <= 8) {
  ...
  return distributeAttractions([...seen.values()], routeKm, 10);
}
  ...
  if (url.pathname === "/api/feedback" && req.method === "POST") {
    ...
    req.on("end", async()=>{
      try {
        ...
        const entry = {ts:new Date().toISOString(), message, contact};
        feedbackStore.push(entry);
        if (feedbackStore.length > 500) feedbackStore.shift();
        let delivered = false; const diag = [];
        const isEmail = /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(contact);
        const fromEmail = isEmail ? contact : "noreply@road-captain.app";
        // --- Canale 1: Web3Forms (chiave gia' configurata in env) ---
        const key = process.env.FEEDBACK_ACCESS_KEY || "7f7450b4-6cc9-4093-ade6-4336d47419e3";
        if (key) {
          try {
            const payload = {
              access_key: key,
            ...
            if (isEmail) payload.replyto = contact;
            const w3 = await getJson("https://api.web3forms.com/submit", {
              method:"POST",
              headers:{"Content-Type":"application/json", "Accept":"application/json"},
              body: JSON.stringify(payload)
            });
            if (w3 && w3.success === true) delivered = true;
            diag.push("web3forms: " + (w3 && (w3.message || (w3.success ? "ok" : "rifiutato")) || "nessuna risposta"));
          } catch (e) { diag.push("web3forms ERRORE: " + (e && e.message || e)); }
        }
        // --- Canale 2 (backup gratuito, nessuna chiave): FormSubmit ---
        // Alla PRIMA richiesta FormSubmit invia una mail di attivazione al destinatario:
        // va aperta una sola volta, poi i feedback arrivano. Il destinatario resta lato server.
        if (!delivered) {
          try {
            const fs = await getJson("https://formsubmit.co/ajax/" + encodeURIComponent(FEEDBACK_TO), {
              method:"POST",
              headers:{"Content-Type":"application/json", "Accept":"application/json"},
              body: JSON.stringify({
                name: "Road Captain by Rhegium",
                email: fromEmail,
                _subject: "Road Captain - nuovo feedback",
                _template: "table",
                message,
                contatto: contact || "(nessuno)"
              })
            });
            const ok = fs && (fs.success === true || fs.success === "true");
            if (ok) delivered = true;
            diag.push("formsubmit: " + (fs && (fs.message || (ok ? "ok" : "in attesa/attivazione")) || "nessuna risposta"));
          } catch (e) { diag.push("formsubmit ERRORE: " + (e && e.message || e)); }
        }
        const diagText = diag.join(" | ") || "nessun canale configurato";
        entry.delivery = {delivered, diag:diagText};   // visibile da /api/admin/feedback per diagnosi
        return send(res, 200, {ok:true, delivered, diag:diagText, note: delivered ? "Feedback inviato." : "Feedback registrato (email non ancora recapitata)."});
      ...
      try {
        ...
        if (cachedPlan) {
          ...
          [pois, routePois] = await Promise.all([
            ...
            collectRouteAttractions(rt.polyline,cum0,rt.distanceKm)
          ]);
