  ...
  for (let km = 15; km < routeKm; km += sampleStep) {
    ...
    for (const poi of pois) {
      ...
      if (loc.distKm <= 0.15) {
        ...
        if (!seen.has(key)) seen.set(key, {...poi, routeKm:loc.km, offsetMeters:Math.round(loc.distKm*1000), onMotorway});
      }
    }
  }
...
const SA_NOISE = new Set(["shop","bar","area","stazione","servizio","di","del","della","sole",
  ...
  if (url.pathname === "/api/plan" && req.method === "POST") {
    ...
    req.on("end", async()=>{
      try {
        ...
        const totalPlannedLong = longStopsCfg.reduce((s,c)=>s+c.minutes, 0);
        ...
        if (longStopsCfg.length) {
          const selDeparture = b.mode === "arrive" ? addMinutes(baseDeparture, -totalPlannedLong) : baseDeparture;
          const assigned = new Set();
          // PASS 1 — ogni sosta lunga si aggancia alla sosta carburante DISTINTA piu' vicina al suo orario.
          for (const cfg of longStopsCfg) {
            ...
            for (const s of valid) { const diff = tdiff(etaForStop(s, selDeparture, false)); if (diff < best) { best = diff; chosen = s; } }
            assigned.add(chosen);
            ...
            if (tdiff(chosenEta) > 90) {
              chosen.longStopWarning = `Nessuna sosta carburante vicino alle ${cfg.time}: la piu' vicina cade alle ${fmtRome(chosenEta).split(", ")[1] || fmtRome(chosenEta)}. Sosta lunga agganciata comunque a quella sosta.`;
            }
          }
...
if (import.meta.url === pathToFileURL(process.argv[1] || "").href) {
  server.listen(PORT,()=>console.log(`Road Captain by Rhegium 5.6.0 on ${PORT}`));
}
