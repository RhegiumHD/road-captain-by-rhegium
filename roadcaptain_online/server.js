...
// Estrae il nome dell'area di servizio ("Villa San Giovanni Est", "La Macchia Ovest")
// da nome/indirizzo, fermandosi prima di codici autostrada/strada/CAP.
function serviceAreaName(name="", address="") {
  const text = `${name} , ${address}`.replace(/\s+/g, " ");
  const m = text.match(/area\s+(?:di\s+)?servizio\s+(.+?)(?=,|\s+(?:a\d{1,2}\b|ss\.?\d|sp\.?\d|via\b|strada\b|km\b|\d{5})|$)/i);
  return (m && m[1].trim()) ? titleCaseIt(m[1]) : "";
}
...
if (import.meta.url === pathToFileURL(process.argv[1] || "").href) {
  server.listen(PORT,()=>console.log(`Road Captain by Rhegium 5.0.0 on ${PORT}`));
}
