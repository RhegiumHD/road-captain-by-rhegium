  ...
  if (url.pathname === "/api/feedback" && req.method === "POST") {
    ...
    req.on("end", async()=>{
      try {
        ...
        let delivered = false;
        const key = process.env.FEEDBACK_ACCESS_KEY;
        if (key) {
          try {
            await getJson("https://api.web3forms.com/submit", {
              method:"POST",
        ...
        return send(res, 200, {ok:true, delivered, note: delivered ? "Feedback inviato." : "Feedback registrato."});
  ...
  if (url.pathname === "/api/plan" && req.method === "POST") {
    ...
    req.on("end", async()=>{
      try {
        ...
        const maxAutonomyForPlan = autonomyKm || 100000;
        ...
        const stopsRaw = planFuelStops({routeKm:rt.distanceKm, pois, stopEveryKm:stopEveryKmEff, forwardWindowKm:b.forwardWindowKm||25, maxAutonomyKm:maxAutonomyForPlan});
        let prevStopKm = 0;
