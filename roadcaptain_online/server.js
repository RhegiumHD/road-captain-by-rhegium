    ...
    req.on("end", async()=>{
      try {
        ...
        const stops = stopsRaw.map(s => {
          ...
          const enriched = {...s, fromPreviousKm: Number((routeKm - prevStopKm).toFixed(1))};
          prevStopKm = routeKm;
          return enriched;
        });
        const validStops = stops.filter(s=>s.status!=="CRITICA").length;
        const stopMinutes = Number(b.stopMinutes || 15);
        const longCfg = b.longStop || {};
        const longStopEnabled = Boolean(longCfg.enabled);
        const longStopAfter = Math.max(1, Number(longCfg.afterStop || 1));
        const longStopMinutes = longStopEnabled ? Math.max(0, Number(longCfg.minutes || 0)) : 0;
        if (longStopEnabled && stops[longStopAfter - 1] && stops[longStopAfter - 1].status !== "CRITICA") {
          stops[longStopAfter - 1].longStopMinutes = longStopMinutes;
        }
        ...
        return send(res,200,{ok:true, mode:GOOGLE_KEY?"Google":"Fallback gratuito", geocoded:{origin,destination,waypoints}, route:rt, tolls, poisFound:pois.length, pois:poisForMap,
...
if (import.meta.url === pathToFileURL(process.argv[1] || "").href) {
  server.listen(PORT,()=>console.log(`Road Captain by Rhegium 3.1.11 on ${PORT}`));
}
