...
async function suggestGoogle(q) {
  // Places API (New) Autocomplete: stessa famiglia di searchText/searchNearby gia' in uso.
  ...
  return (data.suggestions || [])
    ...
    .filter(Boolean)
    .slice(0,6)
    .map(description => ({description, source:"Google"}));
}
...
export function parseMimitFuelPriceCsv(csvText, type="benzina") {
  ...
  // Benzina e gasolio: si confronta il self-service (piu' diffuso e comparabile).
  // GPL e metano: nel CSV MIMIT sono quasi sempre isSelf=0, quindi non si filtra per self.
  const t = String(type || "benzina").toLowerCase();
  const requireSelf = (t === "benzina" || t === "diesel");
  const prices = [];
  for (let i = start; i < lines.length; i++) {
    ...
    if (requireSelf && isSelf !== "1") continue;
...
async function getMimitFuelPrice(type="benzina") {
  ...
  if (!mimitFuelCache.text || now - mimitFuelCache.ts > 6 * 60 * 60 * 1000) {
    ...
    let r;
    try {
      r = await fetch(MIMIT_PRICE_URL, {
        redirect:"follow",
        signal: ctrl.signal,
        headers:{
          "User-Agent":"Mozilla/5.0 (compatible; RoadCaptainByRhegium/1.0; +https://road-captain-by-rhegium.onrender.com)",
  ...
  if (url.pathname === "/api/plan" && req.method === "POST") {
    ...
    req.on("end", async()=>{
      try {
        ...
        const totalMin = rt.durationHours*60 + validStops*stopMinutes + sumAssignedLong - stopMinutes*nAssignedLong;
        ...
        return send(res,200,{ok:true, mode:GOOGLE_KEY?"Google":"Fallback gratuito", geocoded:{origin,destination,waypoints}, route:rt, tolls, tollNotice, poisFound:pois.length, pois:poisForMap, routePois,
          times:{departure:fmtRome(departure),arrival:fmtRome(arrival),stopMinutes:Math.max(0,(validStops-nAssignedLong))*stopMinutes,longStopMinutes:totalLongStopMinutes,totalMinutes:Math.round(totalMin),driveMinutes:Math.round(rt.durationHours*60),pauseMinutes:Math.max(0,Math.round(totalMin-rt.durationHours*60))},
