...
export function addStopEtas(stops, routeDurationMinutes, routeKm, baseDate, mode = "depart", requestedTime = "09:00") {
  ...
  const totalStopMinutes = valid.reduce((sum, s) => sum + Number(s.durationMinutes || 20), 0);
  const totalMinutes = routeDurationMinutes + totalStopMinutes;
  ...
  const withEta = stops.map(s => {
    ...
    const eta = new Date(departure.getTime() + (routeDurationMinutes * driveShare + stopMinutesBefore) * 60000);
    const etaMinuteOfDay = eta.getHours() * 60 + eta.getMinutes();
    const out = { ...s, etaIso: eta.toISOString(), etaMinuteOfDay };
