...
async function googleAutocomplete(input) {
  // 1) Google Places legacy autocomplete
  try {
    const url = "https://maps.googleapis.com/maps/api/place/autocomplete/json?input=" +
      encodeURIComponent(input) + "&language=it&components=country:it&key=" + GOOGLE_KEY;
    const data = await fetchJson(url);
    if (data.status === "OK") {
      return (data.predictions || []).slice(0, 7).map(p => ({
  ...
  // 2) Google Geocoding as robust fallback under the same key restrictions
  try {
    const url = "https://maps.googleapis.com/maps/api/geocode/json?address=" +
      encodeURIComponent(input) + "&region=it&language=it&key=" + GOOGLE_KEY;
    const data = await fetchJson(url);
    if (data.status === "OK") {
  ...
  // 3) Public fallback, so the UI still suggests something if Google is not ready
  try {
    const url = "https://nominatim.openstreetmap.org/search?format=json&limit=7&countrycodes=it&q=" +
...
app.get("/api/config", (req, res) => {
  res.json({
    ...
    mapsScriptUrl: GOOGLE_KEY ? buildGoogleMapsUrl(GOOGLE_KEY) : null
  });
});
...
app.get("/api/suggest", async (req, res) => {
  try {
    ...
    if (!GOOGLE_KEY) {
      const url = "https://nominatim.openstreetmap.org/search?format=json&limit=7&countrycodes=it&q=" + encodeURIComponent(q);
      const data = await fetchJson(url);
      return res.json({
        ok: true,
        items: Array.isArray(data) ? data.map(r => ({ description: r.display_name, placeId: null, source: "Nominatim fallback" })) : []
      });
    }

    res.json({ ok: true, items: await googleAutocomplete(q) });
  } catch (error) {
