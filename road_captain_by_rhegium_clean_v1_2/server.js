...
async function googleAutocomplete(input) {
  ...
  // 1) Google Places API (New): works when the new Places API is enabled.
  try {
  ...
  // 2) Google Places legacy autocomplete.
  try {
    ...
    const data = await fetchJson(url);
    if (data.status === "OK") {
      const items = (data.predictions || []).slice(0, 7).map(p => ({
  ...
  try {
    ...
    const data = await fetchJson(url);
    if (data.status === "OK") {
      const items = (data.results || []).slice(0, 7).map(r => ({
